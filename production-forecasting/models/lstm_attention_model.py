        ...
        if self.scaler is None:
            ...
            if not getattr(self, 'quiet', False):
                try:
                    ...
                    print(
                        f"  [Transform] scaler center/mean={mean_or_center} scale={scale_attr} (type={type(self.scaler).__name__})")
                    # Print a small tail sample to inspect transform effect
                    tail_raw = data_values[-5:] if len(
                        data_values) >= 5 else data_values
                    tail_scaled = data_scaled[-5:] if len(
                        data_scaled) >= 5 else data_scaled
                    print(
                        f"  [Transform] raw tail={tail_raw} scaled tail={tail_scaled}")
                except Exception:
                    pass
    ...
    def predict(self, **kwargs) -> List[float]:
        ...
        if self.scaler is None:
            ...
            if not getattr(self, 'quiet', False):
                try:
                    print(
                        f"  [Transform] preds_scaled_sample={predictions_scaled[:5]} preds_de_scaled_sample={predictions[:5]}")
                except Exception:
                    pass
