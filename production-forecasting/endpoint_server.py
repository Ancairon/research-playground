...
Accepts POST /forecast with JSON payload containing horizon and data,
runs LSTM-Attention tuner and forecaster, returns predictions.
"""
...
def forecast():
    try:
        ...
        if 'data' in data:
            ...
            time_series = pd.read_csv(data['csv_path'], parse_dates=['timestamp'], index_col='timestamp')['value']
        else:
        ...
        if invoke_helper:
            # Run visualization helper with metrics, actuals, and smoothed data if available
            metrics = response.get('metrics') if evaluation else None
            actuals = results.get('actuals') if evaluation else None
            train_window = results.get('train_window')  # lookback + horizon used for training
            raw_train = results.get('train_data') if evaluation else None
            smoothed_train = results.get('smoothed_train_data') if results.get('smoothing_applied') else None
            smoothed_actuals = results.get('smoothed_actuals') if results.get('smoothing_applied') else None
...
def parse_data(data):
    ...
    if isinstance(data, list):
        if data and isinstance(data[0], dict) and 'timestamp' in data[0] and 'value' in data[0]:
            ...
            values = [item[1][0] if isinstance(item[1], list) else item[1] for item in data]
            series = pd.Series(values, index=pd.to_datetime(timestamps, unit='s'))
        else:
...
def parse_csv_data(csv_string):
    ...
    df = pd.read_csv(StringIO(csv_string), parse_dates=['timestamp'], index_col='timestamp')
    return df['value']
