...
def single_shot_evaluation(
    ...
    IMPORTANT: This function uses the END of the data for predictions to ensure that different train_window sizes (from different lookback values) all predict the same final horizon points. The train_window grows BACKWARD into history.
    ...
    if verbose:
        ...
        print(
            f"Training: rows [{train_start}, {train_end}) = {len(train_data)} points")
        print(f"Prediction input: last {lookback} points of training")
        print(
            f"Evaluation: rows [{prediction_start}, {data_end}) = {horizon} points (END of CSV)")

    # Train the model
    if verbose:
        print(f"  Training with {len(train_data)} points...")
        print(
            f"  First train value (raw): {train_data.iloc[0]:.3f}, Last train value (raw): {train_data.iloc[-1]:.3f}")
    ...
    try:
        if getattr(forecaster, 'smoothing_method', None):
            ...
            if isinstance(train_data_for_training, list):
                train_data_for_training = pd.Series(
                    train_data_for_training, index=train_data.index)
        else:
    ...
    if verbose and getattr(forecaster, 'smoothing_method', None):
        try:
            print(
                f"  Training ON SMOOTHED series using method={forecaster.smoothing_method} (window={getattr(forecaster, 'smoothing_window', None)}, alpha={getattr(forecaster, 'smoothing_alpha', None)})")
            print(
                f"  First train value (smoothed): {train_data_for_training.iloc[0]:.3f}, Last train value (smoothed): {train_data_for_training.iloc[-1]:.3f}")
        except Exception:
            pass
    ...
    if verbose:
        ...
        print(
            f"  First pred: {predictions[0]:.3f}, Last pred: {predictions[-1]:.3f}")
    ...
    if verbose:
        ...
        print(
            f"  First actual: {actuals[0]:.3f}, Last actual: {actuals[-1]:.3f}")
    ...
    try:
        if getattr(forecaster, 'smoothing_method', None):
            actuals = apply_smoothing(actuals_orig, method=forecaster.smoothing_method, window=getattr(
                forecaster, 'smoothing_window', 3), alpha=getattr(forecaster, 'smoothing_alpha', 0.2))
    ...
    mbe = np.mean([actual - pred for actual,
                  pred in zip(actuals, predictions)])

    # RMSE (Root Mean Squared Error)
    rmse = np.sqrt(
        np.mean([(actual - pred)**2 for actual, pred in zip(actuals, predictions)]))
    ...
    return {
        ...
        'smoothed_train_data': list(train_data_for_training.values) if hasattr(train_data_for_training, 'values') else list(train_data_for_training),
        ...
        'rmse': rmse,
        'errors': errors,
        'train_window': train_window,  # Include for visualization
        'smoothing_applied': getattr(forecaster, 'smoothing_method', None) is not None,
    }
