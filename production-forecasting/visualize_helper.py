...
def generate_visualization_html(
    ...
    has_timestamps = historical_timestamps is not None and len(historical_timestamps) > 0
    ...
    if has_timestamps:
        ...
        pred_labels = list(range(len(historical_values), len(historical_values) + len(predictions)))
        all_labels = hist_labels + pred_labels
        x_axis_type = 'linear'
    
    # Determine if smoothing was applied
    has_smoothing = smoothed_historical is not None and len(smoothed_historical) > 0
    has_actuals = actuals is not None and len(actuals) > 0
    has_smoothed_actuals = smoothed_actuals is not None and len(smoothed_actuals) > 0
    ...
    smoothed_hist_data = (list(smoothed_historical) + [None] * len(predictions)) if has_smoothing else None
    ...
    actuals_data = [None] * len(historical_values) + (list(actuals) if has_actuals else [None] * len(predictions))
    
    # Smoothed Actuals (if available)
    smoothed_actuals_data = ([None] * len(historical_values) + list(smoothed_actuals)) if has_smoothed_actuals else None
    ...
    # Legend info
    legend_parts = [f"<strong>Historical:</strong> {len(historical_values)} points"]
    if has_smoothing:
        legend_parts[-1] += " (raw + smoothed)"
    legend_parts.append(f"<strong>Predictions:</strong> {len(predictions)} points")
    if has_actuals:
...
if __name__ == '__main__':
    ...
    html = generate_visualization_html(historical, predictions, metrics, timestamps)
