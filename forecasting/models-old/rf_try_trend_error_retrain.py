...
def main():
    ...
    try:
        while True:
            ...
            if retrain:
                print("Retraining model due to trend error …")
                model = train_rf_multi(df, args.lag, args.horizon)
            fc = forecast_multi(df, model, args.lag, args.horizon)
            H = args.horizon
            recent_actuals = df['value'].iloc[-H:].values
            x = np.arange(H)
            slope_act, _ = np.polyfit(x, recent_actuals, 1)
            y_fc = fc.values
            slope_fc, _ = np.polyfit(x, y_fc, 1)
            trend_err = abs(slope_fc - slope_act)
            print(
                f"Trend slopes → actual={slope_act:.3f}, forecast={slope_fc:.3f}, error={trend_err:.3f}")
            retrain = (trend_err > TREND_THRESHOLD)
