# hw_model.py - Holt-Winters Exponential Smoothing
import argparse
import pandas as pd
import numpy as np
...
parser = argparse.ArgumentParser(
    description="Holt-Winters TS forecaster with live plotting and adaptive retraining"
)
...
parser.add_argument('--trend',           default='add',
                    help='Trend component: "add", "mul", or None')
parser.add_argument('--seasonal',        default=None,
                    help='Seasonal component: "add", "mul", or None')
parser.add_argument('--seasonal_periods', type=int, default=None,
...
def train_hw_model(series, trend='add', seasonal=None, seasonal_periods=None):
    ...
    try:
        ...
        try:
            return ExponentialSmoothing(series, trend=None, seasonal=None).fit()
        except (ValueError, TypeError):
            # Ultimate fallback - return a simple model that just predicts last value
            class SimpleModel:
                def __init__(self, last_value):
                    self.last_value = last_value
                def forecast(self, steps):
                    return [self.last_value] * steps
            return SimpleModel(series.iloc[-1])
...
def main(
    ...
    model = train_hw_model(initial_data, trend=trend, seasonal=seasonal, seasonal_periods=seasonal_periods)
    train_time = time.time() - start_train
    if not quiet:
        print(f"Initial Holt-Winters training time: {train_time:.3f}s")
    ...
    while True:
        ...
        if prev_pred is not None:
            ...
            if retrain_threshold is not None and len(errors) == errors.maxlen:
                ...
                if avg_err > retrain_threshold:
                    if not quiet:
                        print(f"\n\nRetraining Holt-Winters model, avg err={avg_err:.4f}")
                    holdback += 1
                    if holdback == 5:
                        holdback = 0
                        if not quiet:
                    ...
                    try:
                        model = train_hw_model(live_df['value'], trend=trend, seasonal=seasonal, seasonal_periods=seasonal_periods)
        ...
        if not quiet:
            ...
            ax.plot(future, preds,   'r--o', label='Holt-Winters Forecast')
